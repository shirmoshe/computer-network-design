...
void *handle_client(void *arg) {
    ...
    while (1) {
        ...
        switch (type) {
            case 0: {
				int temp =-1;
				sscanf(buffer, "Message Type:(0)Client joining, my id: %d", &temp);
				if (client_arr[temp - 1] == 0){
					client_arr[temp - 1] = 1;
					// type JOIN
					...
					char back_msg[BUFFER_SIZE];
					sprintf(back_msg,"Message Type:(8),Id is occupied, Please choose anther one");
					printf("--> Send: %s\n", back_msg);
					send(client_socket, back_msg, strlen(back_msg), 0);  // Send welcome msg
        ...
        for (i = 0; i < MAX_CLIENTS; ++i) {
            if (clients[i].socket != -1) {
                ...
                if (difftime(now, clients[i].last_activity) >= TIMEOUT) {
                    ...
                    sprintf(goodbye_msg, "Message Type:(9) GoodBye!\n");
					client_arr[i - 1] == 0;
                    printf("--> sent: %s\n", goodbye_msg);
